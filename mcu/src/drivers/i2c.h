...
void i2c_write(uint16_t slave_addr, uint8_t reg_addr, uint8_t len, uint8_t *data); // write len bytes to slave registers
uint8_t *i2c_read(uint16_t slave_addr, uint8_t reg_addr, uint8_t len); // read len bytes from slave registers
